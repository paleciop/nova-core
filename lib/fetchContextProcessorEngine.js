...
console.log('IS DEBUG', utils.isDebug());
  /**
  ...
  const execute = function execute(
    ...
    const statistics = utils.isDebug() && { originalContentModel , dependent: [], independent: [] };
      ...
      contextProcessorArray.reduce(
        ...
        (thePromise, contextProcessorArray) =>
          thePromise.then(() =>
            ...
            if (utils.isDebug()) {
              const contentModelCopy = JSON.parse(JSON.stringify(contentModel));
              delete contentModelCopy.statistics;
              contentModel.statistics.dependent.push({
    ...
    const independentContextProcessorPromises = Promise.all(
      arrangedContextProcessors.independent.map(cp => processCP(cp)) //TODO add debug logic here
    );
