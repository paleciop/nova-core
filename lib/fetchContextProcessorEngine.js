...
module.exports = async function fetchContextProcessorEngine(
  ...
  const execute = function execute(
    ...
    const contentModel = Object.assign(
      ...
      JSON.parse(JSON.stringify(originalContentModel))
    ); // Deep clone the content model and add the statistics object
    ...
    const processCP = contextProcessor => {
      ...
      try{
        ...
        ).catch(error => handleError(error)); // catch async errors
    ...
    const arrangedContextProcessors = Object.values(contextProcessors) // Get the Context Processor Objects.
      ...
      .reduce(
        (arrangedContextProcessors, contextProcessor) => {
          ...
          if (typeof contextProcessor.priority !== "undefined") {
            ...
            // If the priority is undefined, the context processor is considered independent, so we just add it to the independent.
