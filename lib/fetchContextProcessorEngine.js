...
module.exports = async function fetchContextProcessorEngine(
  ...
  const execute = function execute(
    ...
    const contentModel = Object.assign({ statistics: { dependent: [], independent: [] } }, originalContentModel);
    const processCP = contextProcessor => {
      if (contextProcessor.getMatch) { //TODO move it elsewhere since getMatch is not really a core functionality
    ...
    const arrangedContextProcessors = Object.values(loadedContextProcessors) // Get the Context Processor Objects.
      ...
      .reduce(
        (arrangedContextProcessors, contextProcessor) => {
          ...
          if (contextProcessor.priority === null || (typeof contextProcessor.priority !== "undefined")) {
    ...
    contentModel.statistics.independent = arrangedContextProcessors.independent.map(cp => cp.name);
    contentModel.statistics.dependent = arrangedContextProcessors.dependent.map(array => ({
    ...
    return Promise.all([independentContextProcessorPromises, dependentContextProcessorPromises]).then(() => {
  ...
  const loadContextProcessorsFromObjects = async function loadContextProcessorsFromObjects(
    ...
    log("Loading Context Processors from objects: ");
    log(contextProcessorObjects);
    let i = 0;
    contextProcessorObjects.forEach(contextProcessorsObj => {
      if (utils.isValidContextProcessor(contextProcessorsObj)) {
        ...
        const contextProcessorName = hasName && contextProcessorsObj.name || 'AnonymousContextProcessor' + i++;
        loadedContextProcessors[contextProcessorName] = !hasName
