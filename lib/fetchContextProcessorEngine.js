...
module.exports = async function fetchContextProcessorEngine(
  ...
  const execute = function execute(executionContext, contentModel = {}) {
     ...
     */
    const accepts = contextProcessor => contextProcessor.accepts(executionContext);
    ...
    return Object.values(contextProcessors) // Get the Context Processor Objects.
      ...
      .reduce((arrangedContextProcessors, contextProcessor) => {
        ...
        // Add Context Processor name to the statistics object in the content model.
        if (Array.isArray(contentModel.statistics)) {
          contentModel.statistics.push(contextProcessor.name);
        } else {
          contentModel.statistics = [contextProcessor.name];
        }
          ...
          thePromise.then(() =>
            Promise.all(
              contextProcessorArray.map(contextProcessor =>
                ...
                Promise.resolve(contextProcessor.process(executionContext, contentModel))
              )
            )
          ),
