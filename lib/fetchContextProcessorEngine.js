...
module.exports = async function fetchContextProcessorEngine(
  ...
  /**
   *
   * @param {Object} globalExecutionContext the executionContext.
   ...
   */
  const execute = function execute(globalExecutionContext, contentModel = {}) {
    let debug = {};
    if (isDebug) {
    ...
    const processCP = contextProcessor => {
      ...
      const executionContext = Object.assign({}, globalExecutionContext, {
        config: contextProcessor.getConfig()
      });
      try {
        // Wrap the results of .process() with Promise.resolve() to ensure a Promise is returned.
        return Promise.resolve(
          contextProcessor.process(executionContext, contentModel)
    ...
    const accepts = contextProcessor =>
      contextProcessor.accepts(globalExecutionContext);
